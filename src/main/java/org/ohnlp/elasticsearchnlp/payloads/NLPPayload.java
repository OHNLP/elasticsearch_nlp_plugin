...
public class NLPPayload {
    ...
    public NLPPayload() {
        this(new byte[] {0b0, 0b0, 0b0, 0b0, 0b0, 0b0, 0b0});
    }
    ...
    public NLPPayload(ByteBuffer buff, int offset) {
        ...
        int embeddingSize = buff.getInt(offset + 3);
        FloatBuffer embeddingsBuff = FloatBuffer.allocate(embeddingSize);
        for (int i = 0; i < embeddingSize; i++) {
            embeddingsBuff.put(i, buff.getFloat(offset + 7 + (i * EMBEDDING_BYTE_SIZE))); // Read in this way manually to guarantee ordering
        }
     ...
     */
    public byte[] toBytes() {
        ByteBuffer buff = ByteBuffer.allocate(6 + embeddings.length * EMBEDDING_BYTE_SIZE).order(ByteOrder.LITTLE_ENDIAN);
        // ConText Byte
        ...
        buff.putInt(3, embeddings.length);
        // Transpose the embeddings in-sequence onto the bytebuffer. We do this to avoid endianness issues
        for (int i = 0; i < embeddings.length; i++) {
            buff.putFloat(7 + (i * EMBEDDING_BYTE_SIZE), embeddings[i]);
        }
        byte[] ret = new byte[6 + embeddings.length * EMBEDDING_BYTE_SIZE];
        buff.get(ret);
        return ret;
    }
